...
local NetFile = "http://bilbao.lima-city.de/BilbaoEzreal.lua" 
local LocalFile = BOL_PATH.."Scripts\\BilbaoEzreal.lua" 
-------/Auto update-------
		...
		Menu:addSubMenu("Extra", "extra")
			...
			Menu:addSubMenu("Special", "specl")
				...
				Menu.specl:addSubMenu("W-Options", "wopt")
					Menu.specl.wopt:addParam("wopt1", "Choose your W usage", SCRIPT_PARAM_LIST, 1, { "MainTarget", "AllyMostAD" })
...
if Menu.specl.wopt.wopt1 == 1 then
	...
	if Menu.ta.co == 3 and vpredicfile then 	
			if ValidTarget(Target) and not Target.dead and Target.visible and GetDistance(Target) < Wrange then
				local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(Target, Wdelay, Wwidth, Wrange, Wspeed, myHero, false)
...
local currAllyMostAd =  _GetAllyMostAd(Wrange)
if currAllyMostAd ~=nil and GetDistance(currAllyMostAd) < Wrange then
	main_cast_pos = currAllyMostAd
end
end
...
function _GetAllyMostAd(range)
	local MostAd, MostAdUnit = 1, nil
	for i, currAlly in pairs(GetAllyHeroes()) do		
        if GetDistance(currAlly) < range and not currAlly.dead and currAlly.addDamage >= MostAd then				
			MostAd =  currAlly.addDamage
			MostAdUnit = currAlly
        end
	end
return MostAdUnit
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then	
			if ValidTarget(Target) and not Target.dead and Target.visible and GetDistance(Target) < Rrange then
				local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(Target, Rdelay, Rwidth, Rrange, Rspeed, myHero, false)
				if HitChance >= minVPhit and Target.visible then
					if CastPosition ~= nil then
						main_cast_pos = CastPosition						
					end
				end	
