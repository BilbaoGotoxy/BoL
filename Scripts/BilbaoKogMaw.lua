...
local NetFile = "http://bilbao.lima-city.de/BilbaoKogMaw.lua" 
local LocalFile = BOL_PATH.."Scripts\\BilbaoKogMaw.lua" 
-------/Auto update-------
...
_G.freepredicfile = true
	...
	local Rrange, Rwidth, Rspeed, Rdelay = 1400, 200, 2000, 0.47
...
function _loadP()
	if prodicfile then
		...
		ProdictionE = Prodiction:AddProdictionObject(_E, Qrange, Espeed, Edelay, Ewidth) 
...
function _initiateTS()
	...
	rts = TargetSelector(TARGET_LOW_HP, Rrange)
	rts.name = "Ultimate"	
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then
			if ValidTarget(Target) and not Target.dead and Target.visible and GetDistance(Target) < Rrange then
				local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(Target, Rdelay, Rwidth, Rrange, Rspeed, myHero, false)
