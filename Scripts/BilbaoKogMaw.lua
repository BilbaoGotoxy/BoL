...
if FileExist(SCRIPT_PATH..'Common/SOW.lua') then
	...
	-------Skills info-------	
	local Qrange, Qwidth, Qspeed, Qdelay = 975, 70, 1200, 0.5		
	local Erange, Ewidth, Espeed, Edelay = 1200, 120, 1200, 0.5	
	local Rrange, Rwidth, Rspeed, Rdelay = 1200, 200, 2125, 0.47
	local RrangeCollection = {1200, 1500, 1800}
	local RBuffCount = 0
		...
		-----------------------------------------------------------------------------------------------------				
			Menu:addSubMenu("Special", "specl")
				...
				Menu.specl:addSubMenu("R-Options", "ropt")
					...
					Menu.specl.ropt:addParam("ropt1", "[Harrass] Max R stucks",  SCRIPT_PARAM_SLICE, 4, 0, 11, 0)
					Menu.specl.ropt:addParam("ropt2", "[Rotation] Max R stucks",  SCRIPT_PARAM_SLICE, 4, 0, 11, 0)
					Menu.specl.ropt:addParam("ropt3", "[LaneClear] Max R stucks",  SCRIPT_PARAM_SLICE, 4, 0, 11, 0)
		-----------------------------------------------------------------------------------------------------
...
function _update_rrange()
local rlvl = myHero:GetSpellData(_R).level
	if rlvl > 0 then
		Rrange = RrangeCollection[rlvl]
	end
end
...
if (Menu.keys.permrota or Menu.keys.okdrota) then
			...
			if Menu.rota.useW and WReady then
				cast_pred_w()	
			end
			...
			if Menu.rota.useR and canRrota and RReady then
				if VIP_USER then
					if RBuffCount <= Menu.specl.ropt.ropt1 then
						cast_pred_r()
					end
				else
					cast_pred_r()
...
if (Menu.keys.permhrs or Menu.keys.okdhrs) then	
			...
			if Menu.harrass.autohrsW and WReady then				
				cast_pred_w()	
			end
			...
			if Menu.harrass.autohrsR and canRhrs and RReady then				
				if VIP_USER then
					if RBuffCount <= Menu.specl.ropt.ropt2 then
						cast_pred_r()
					end
				else
					cast_pred_r()
...
if Menu.keys.okdlc then
	...
	if Menu.laneclear.autolcW and WReady then
		cast_lc_w()
	end
	...
	if Menu.laneclear.autolcR and canRlc and RReady then
			...
			if VIP_USER then
				if RBuffCount <= Menu.specl.ropt.ropt3 then
					cast_lc_r()
				end
			else
				cast_lc_r()
...
local ETAR = nil
if Menu.specl.eopt.ets == 1 then
	ETAR = Target
elseif Menu.specl.eopt.ets == 2 then
	ETAR = ets.target
end
local main_cast_pos = nil
	if Menu.ta.co == 1 then	
	...
	if Menu.ta.co == 2 and VIP_USER then
		if ETAR ~=nil and ETAR.visible and GetDistance(ETAR) < Erange then
			local Position = VipPredictionR:GetPrediction(ETAR)
			if Position ~= nil then
				main_cast_pos = Position				
			end		
		end	
	...
	if Menu.ta.co == 4 and prodicfile then
		if ValidTarget(ETAR) and not ETAR.dead and ETAR.visible and GetDistance(ETAR) < Erange then
			main_cast_pos = ProdictionR:GetPrediction(ETAR)
		end
	end	
