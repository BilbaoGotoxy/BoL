...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then
			if ValidTarget(QTAR) and not QTAR.dead and QTAR.visible and GetDistance(QTAR) < Qrange then
				local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(QTAR, Qdelay, Qwidth, Qrange, Qspeed, myHero, false)
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then	
			if ValidTarget(WTAR) and not WTAR.dead and WTAR.visible and GetDistance(WTAR) < Wrange then
				local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(WTAR, Wdelay, Wwidth, Wrange, Wspeed, myHero, false)
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then	
			if ValidTarget(ETAR) and not ETAR.dead and ETAR.visible and GetDistance(ETAR) < Erange then		
				local Position, HitChance = VP:GetPredictedPos(ETAR, Edelay, Espeed, myHero, false)
				if HitChance >= minVPhit and ETAR.visible and GetDistance(Position) < Erange then
					if Position ~= nil then
						main_cast_pos = Position						
					end
				end	
	...
	if Menu.ta.co == 4 and prodicfile then
		if ValidTarget(ETAR) and not ETAR.dead and ETAR.visible and GetDistance(ETAR) < Wrange then
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then  	
			if ValidTarget(RTAR) and not RTAR.dead and RTAR.visible and GetDistance(RTAR) < Rrange then
				local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(RTAR, Rdelay, Rwidth, Rrange, Rspeed, myHero, true)
