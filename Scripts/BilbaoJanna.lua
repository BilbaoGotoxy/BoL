		...
		-----------------------------------------------------------------------------------------------------				
			Menu:addSubMenu("Special", "specl")
				...
				Menu.specl:addSubMenu("E-Options", "eopt")
					Menu.specl.eopt:addParam("eopt1", "Buff Order", SCRIPT_PARAM_LIST, 1, { "Self", "MostAD", "LowHP", "MostAS" })

				Menu.specl:addSubMenu("R-Options", "ropt")
					Menu.specl.ropt:addParam("rts", "AutoR", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.ropt:addParam("ropt1", "If more then x-Enemys",  SCRIPT_PARAM_SLICE, 2, 0, 5, 0)
					Menu.specl.ropt:addParam("ropt2", "In this Range",  SCRIPT_PARAM_SLICE, 300, 0, 700, 0)
	...
	for i, enemy in ipairs(GetEnemyHeroes()) do		
		if enemy ~= nil and not enemy.dead and enemy.visible then		
			...
			if WReady and QReady then
				DrawLineHPBar((_dmg("W", enemy)+_dmg("Q", enemy)), currLine, "QW: "..(_dmg("Q", enemy)+_dmg("W", enemy))..dmgtyp, enemy, true)
				currLine = currLine + 1
			end
		end
	end	
...
local main_cast_pos = nil
	...
	if Menu.ta.co == 3 and vpredicfile then 	
			if ValidTarget(LOCAL_TAR) and not LOCAL_TAR.dead and LOCAL_TAR.visible and GetDistance(LOCAL_TAR) < Qrange then
				local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(LOCAL_TAR, Qdelay, Qwidth, Qrange, Qspeed, myHero, false)
...
MarkIt = LOCAL_TAR
	...
	if ValidTarget(LOCAL_TAR, Wrange*0.95) then
...
if Menu.specl.eopt.eopt1 == 2 then
	local ADUnit, ADCount = nil, 0
	for i, ally in ipairs(GetAllyHeroes()) do
		if not ally.dead and GetDistance(ally) < Erange*0.95 then
			if ally.addDamage >= ADCount then
				ADUnit = ally
				ADCount = ally.addDamage
			end
		end
	end
	if ADCount > 0 then
...
elseif Menu.specl.eopt.eopt1 == 3 then
	local HPUnit, HPCount = nil, 50000
	for i, ally in ipairs(GetAllyHeroes()) do
		if not ally.dead and GetDistance(ally) < Erange*0.95 then
			if ally.health <= HPCount then
				HPUnit = ally
				HPCount = ally.health
			end
		end
...
elseif Menu.specl.eopt.eopt1 == 4 then
	local ASUnit, ASCount = nil, 0
	for i, ally in ipairs(GetAllyHeroes()) do
		if not ally.dead and GetDistance(ally) < Erange*0.95 then
			if ally.attackSpeed >= ASCount then
				ASUnit = ally
				ASCount = ally.attackSpeed
			end
		end
	end
	if ASCount > 0 then
	...
	CastSpell(_E, CastEOn or myHero)
end


function cast_pred_r()
if not RReady or not Menu.specl.ropt.rts then return end
local ECount = CountEnemyHeroInRange(Menu.specl.ropt.ropt2, myHero)
	if ECount >= Menu.specl.ropt.ropt1 then
...
if not EReady then return end
		for i, minion in pairs(MyMinionManager.objects) do 
			if minion ~= nil and minion.valid and minion.team ~= myHero.team and GetDistance(minion) < MyBasicRange*0.9 and not minion.dead and minion.visible then	
				CastSpell(_E, myHero)				
			end
		end
end
