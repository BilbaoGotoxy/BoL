...
if FileExist(SCRIPT_PATH..'Common/SOW.lua') then
	...
	-------Skills info-------	
	local Qrange, Qwidth, Qspeed, Qdelay = 875, 200, 1750, 0.5	
	local Wrange, Wwidth, Wspeed, Wdelay = 725, 0, 1100, 0.5	
	local Erange, Ewidth, Espeed, Edelay = 800, 0, 7777, 0.5	
	local Rrange, Rwidth, Rspeed, Rdelay = 2550, 550, 1200, 0.5
		...
		-----------------------------------------------------------------------------------------------------				
			Menu:addSubMenu("Special", "specl")
				...
				Menu.specl:addSubMenu("W-Options", "wopt")
					...
					Menu.specl.wopt:addParam("wopt1", "Force Bounce", SCRIPT_PARAM_ONOFF, false)
					
				Menu.specl:addSubMenu("E-Options", "eopt")					
					Menu.specl.eopt:addParam("eopt1", "Buff Order", SCRIPT_PARAM_LIST, 1, { "Self", "MostAD" })
...
function AddProtectionMenu()
	Menu:addSubMenu("Protection", "prot")
		...
		Menu.prot:addParam("user", "Use R", SCRIPT_PARAM_ONOFF, false)
...
function _draw_predDmg()
	...
	for i, enemy in ipairs(GetEnemyHeroes()) do		
		if enemy ~= nil and not enemy.dead and enemy.visible then		
			...
			if RReady and QReady and not EReady then
...
local GoAwayW = {target = nil, status = 5}
	...
	local LOCAL_TAR = GoAwayQ.target
	if Menu.ta.co == 1 then
		if LOCAL_TAR ~=nil and LOCAL_TAR.visible and GetDistance(LOCAL_TAR) < Qrange then
	...
	if Menu.ta.co == 3 and vpredicfile then
			if ValidTarget(LOCAL_TAR) and not LOCAL_TAR.dead and LOCAL_TAR.visible and GetDistance(LOCAL_TAR) < Qrange then
				local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(LOCAL_TAR, Qdelay, Qwidth, Qrange, Qspeed, myHero, false)
				if HitChance >= minVPhit and LOCAL_TAR.visible and GetDistance(CastPosition) < Qrange then
	...
	if Menu.ta.co == 4 and prodicfile then
		if ValidTarget(LOCAL_TAR) and not LOCAL_TAR.dead and LOCAL_TAR.visible and GetDistance(LOCAL_TAR) < Qrange then
			main_cast_pos = ProdictionQ:GetPrediction(LOCAL_TAR)
	...
	if GoAwayR.status < 5 and ValidTarget(GoAwayR.target, Rrange) and RReady then		
		local main_cast_pos = nil
		local LOCAL_TAR = GoAwayR.target
	if Menu.ta.co == 1 then
	...
	if Menu.ta.co == 3 and vpredicfile then	
			if ValidTarget(LOCAL_TAR) and not LOCAL_TAR.dead and LOCAL_TAR.visible and GetDistance(LOCAL_TAR) < Rrange then
				local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(LOCAL_TAR, Rdelay, Rwidth, Rrange, Rspeed, myHero, false)
...
function _GetNextEnemyTo(unit)
local ClosestUnit, ClostestDist = nil, 50000
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if not enemy.dead and enemy.visible and GetDistanceSqr(unit, enemy) <= ClostestDist and not Menu.prot.ign[enemy.charName]  then
				ClosestUnit = enemy
				ClostestDist = GetDistanceSqr(unit, enemy)
			end
		end
...
MarkIt = LOCAL_TAR
local main_cast_target = nil

if Menu.specl.wopt.wopt1 then 
	local HeroCount = 0
	for i = 1, heroManager.iCount, 1 do
        local hero = heroManager:getHero(i)
        if not hero.dead and GetDistanceSqr(LOCAL_TAR, hero) <= 375 then
            HeroCount = HeroCount + 1
        end
    end
	if HeroCount >= 1 then
		main_cast_target = LOCAL_TAR
	end
else
	main_cast_target = LOCAL_TAR
end

	if main_cast_target ~= nil and WReady then		
			_castSpell(_W, nil, nil, main_cast_target)		
	end
end
...
local ETAR = nil
local self = GetMyHero()

if Menu.specl.eopt.eopt1 == 1 then
	ETAR = GetMyHero()
else
	local MostAdAmount, MostAdUnit = 0, nil
	for i = 1, heroManager.iCount, 1 do
        local hero = heroManager:getHero(i)
        if not hero.dead and GetDistanceSqr(self, hero) <= Erange and hero.team == self.team and hero.addDamage >= MostAdAmount then
            MostAdAmount = hero.addDamage
			MostAdUnit = hero
        end
    end
	if MostAdUnit ~= nil then ETAR = MostAdUnit end
	
end
	if ETAR ~= nil and EReady then		
			CastSpell(_E, ETAR)		
	end
...
if not WReady then return end
		for i, minion in pairs(MyMinionManager.objects) do 
			if minion ~= nil and minion.valid and minion.team ~= myHero.team and GetDistance(minion) < Wrange and not minion.dead and minion.visible then		
				_castSpell(_W, minion.x, minion.z, nil)
			end
		end
end
...
function cast_lc_e()
if not EReady then return end
_castSpell(_E, nil, nil, GetMyHero())
end
