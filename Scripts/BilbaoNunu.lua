...
if FileExist(SCRIPT_PATH..'Common/SOW.lua') then
	...
	-------Skills info-------	
	local Qrange, Qwidth, Qspeed, Qdelay = 125, 70, 1800, 0.25	
	local Wrange = 700
	local Erange, Ewidth, Espeed, Edelay = 550, 70, 1800, 0.25	
	local Rrange, Rwidth, Rspeed, Rdelay = 645, 1300, 7777, 0.1
		...
		-----------------------------------------------------------------------------------------------------				
			Menu:addSubMenu("Special", "specl")
				...
				Menu.specl:addSubMenu("Q-Options", "qopt")
					Menu.specl.qopt:addParam("qopt1", "Use to heal", SCRIPT_PARAM_ONOFF, false)
				
				Menu.specl:addSubMenu("W-Options", "wopt")					
					Menu.specl.wopt:addParam("info", "W Buff Order", SCRIPT_PARAM_INFO, "")
					Menu.specl.wopt:addParam("wopt1", "Buff ADC", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt2", "Buff Allys", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt5", "Buff SuperMinion", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt6", "Buff CannonMinion", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt7", "Buff MeleeMinion", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt8", "Buff RangeMinion", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.wopt:addParam("wopt9", "Buff Self", SCRIPT_PARAM_ONOFF, false)
				...
				Menu.specl:addSubMenu("R-Options", "ropt")
					Menu.specl.ropt:addParam("ropt1", "KS with R", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.ropt:addParam("info", " ", SCRIPT_PARAM_INFO, "")
					Menu.specl.ropt:addParam("ropt2", "AutoCastR if", SCRIPT_PARAM_ONOFF, false)
					Menu.specl.ropt:addParam("ropt2sliderrange", "Count Range",  SCRIPT_PARAM_SLICE, 500, 0, 625, 0)
					Menu.specl.ropt:addParam("ropt2sliderenemy", "AutoCastR if",  SCRIPT_PARAM_SLICE, 2, 0, 5, 0)
...
function _initiateTS()
	...
	ets = TargetSelector(TARGET_LOW_HP, Erange, DAMAGE_MAGI)
	ets.name = "E Target"
...
if not Menu.specl.qopt.qopt1 then return end
...
local WTargetUnit, WTargetStatus = nil, 10

		for i, ally in ipairs(GetAllyHeroes()) do
			if ally ~= nil and GetDistance(ally) < Wrange and not ally.dead and ally.health > 100 then
				if adclist[ally.charName] ~= nil and Menu.specl.wopt.wopt1 then
					WTargetUnit = ally
					WTargetStatus = 1
				end
				if Menu.specl.wopt.wopt2 and WTargetStatus > 2 then
					WTargetUnit = ally
					WTargetStatus = 2
				end
			end
	...
	if WTargetStatus < 2  then
		for i, minion in pairs(MyMinionManager.objects) do
			if minion ~= nil and GetDistance(minion) < Wrange and minion.valid and minion.team == myHero.team and not minion.dead and minion.visible and (minion.health > (minion.maxHealth/2)) then
				if minion.charName:find("Minion_Wizard") and Menu.specl.wopt.wopt8 and WTargetStatus > 8 then
					WTargetUnit = minion
					WTargetStatus = 8
				end			
				if minion.charName:find("Minion_Basic") and Menu.specl.wopt.wopt7 and WTargetStatus > 7 then
					WTargetUnit = minion
					WTargetStatus = 7
				end		
				if minion.charName:find("Minion_MechCannon") and Menu.specl.wopt.wopt6 and WTargetStatus > 6 then
					WTargetUnit = minion
					WTargetStatus = 6
				end				
				if minion.charName:find("Minion_MechMelee") and Menu.specl.wopt.wopt6 and WTargetStatus > 5 then
					WTargetUnit = minion
					WTargetStatus = 5
				end				
			end		
	...
	if WReady then
		if WTargetUnit ~= nil then
			CastSpell(_W, WTargetUnit)
		elseif Menu.specl.wopt.wopt9 then
			CastSpell(_W, myHero)
		end
	end
end
...
MarkIt = LOCAL_TAR
	...
	if ValidTarget(LOCAL_TAR, Erange*0.99) then
...
if Menu.specl.ropt.ropt1 then
	for i, enemy in ipairs(GetEnemyHeroes()) do
		if not enemy.dead and GetDistance(enemy) < 1285 and enemy.health < getDmg("R", enemy, myHero) then
...
if Menu.specl.ropt.ropt2 then
	local CEnemys = CountEnemyHeroInRange(Menu.specl.ropt.ropt2sliderrange, myHero)
	if CEnemys >= Menu.specl.ropt.ropt2sliderenemy then
		_castSpell(_R, myHero.x, myHero.z, nil)
	end
elseif CEnemys >= 1 and not Menu.specl.ropt.ropt2 then
	_castSpell(_R, myHero.x, myHero.z, nil)
end
